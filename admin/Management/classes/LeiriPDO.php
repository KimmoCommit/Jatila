...
class LeiriPDO {
	...
	public function getAllLeirit() {
		$sql = "SELECT leirit.name as name,leirit.startTime as startTime, leirit.endTime as endTime, leirit.price as price, opettajat.firstname as firstname, opettajat.lastname as lastname
		FROM leirit 
		INNER JOIN opettajat ON leirit.teacher_id = opettajat.id ORDER BY leirit.startTime ASC;
		";
		...
		while ($row = $stmt->fetchObject() ) {
			$leiri = new Leiri();
			$leiri->setName(utf8_encode($row->name));
			$leiri->setStartTime($row->startTime);
			$leiri->setEndTime($row->endTime);
			$leiri->setPrice($row->price);
			$leiri->setTeacherFname($row->firstname);
			$leiri->setTeacherLname($row->lastname);
